...
def test_slew_translate(squareCamera, raOffset, decOffset):
    ...
    compare_coordsLists(squareCamera.get_coordsList(),
                        squareCameraSlew.get_coordsList())
...
def test_ditherDict_theoretical_2D(squareCamera, raOffset,
                                   decOffset, degrees):
    slewList = []
    slewList.append(dither.Slew(raOffset=raOffset,
                                decOffset=decOffset,
                                degrees=degrees))
    slewList.append(dither.Slew(raOffset=raOffset * 2,
...
def test_ditherDict_theoretical_3D(squareCamera, raOffset, decOffset, degrees):
    ...
    slewList.append(dither.Slew(raOffset=raOffset * 2,
                                decOffset=decOffset * 2,
                                degrees=degrees * 2))
    slewList.append(dither.Slew(raOffset=raOffset * 3,
                                decOffset=decOffset * 3,
                                degrees=degrees * 3))
...
def test_ditherDict_hardcoded(squareCamera, raOffset,
                              decOffset, degrees, ditherDict):
    slewList = []
    for i in range(len(ditherDict)):
        slewList.append(dither.Slew(raOffset=raOffset * i,
                                    decOffset=decOffset * i,
                                    degrees=degrees * i))
