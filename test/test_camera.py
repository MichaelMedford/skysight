...
def test_squareCamera_ra_expand_collapse(squareCoords,
                                         squareCamera,
                                         raOffset,
                                         decOffset):
    coordsList = []
...
def test_squareCamera_difference(squareCamera, raOffset, decOffset, area):
    squareCamera2 = squareCamera.copy()
    squareCamera2.translate(raOffset=raOffset, decOffset=decOffset)
    assert np.isclose(squareCamera.difference(squareCamera2).get_area(),
                      area,
                      atol=1e-2)
    assert np.isclose(squareCamera2.difference(squareCamera).get_area(),
                      area,
                      atol=1e-2)
...
def test_squareCamera_intersect_three(squareCamera, raOffset, decOffset, area):
    ...
    assert np.isclose(squareCamera.intersect(squareCamera2).
                      intersect(squareCamera3).get_area(),
                      area,
                      atol=1e-2)
...
def test_squareCamera_union_three(squareCamera, raOffset, decOffset, area):
    ...
    squareCamera3 = squareCamera.copy()
    squareCamera3.translate(raOffset=raOffset * 2, decOffset=decOffset * 2)
    assert np.isclose(squareCamera.
                      union(squareCamera2).
                      union(squareCamera3).get_area(), area, atol=1e-2)
...
def test_squareCamera_difference_three(squareCamera,
                                       raOffset,
                                       decOffset,
                                       area1,
                                       area2):
    ...
    squareCamera3 = squareCamera.copy()
    squareCamera3.translate(raOffset=raOffset * 2, decOffset=decOffset * 2)
    assert np.isclose(squareCamera.difference(squareCamera2).
                      difference(squareCamera3).get_area(), area1, atol=1e-2)
    assert np.isclose(squareCamera.difference(squareCamera3).
                      difference(squareCamera2).get_area(), area1, atol=1e-2)
    assert np.isclose(squareCamera2.difference(squareCamera3).
                      difference(squareCamera).get_area(), area2, atol=1e-2)
    assert np.isclose(squareCamera2.difference(squareCamera).
                      difference(squareCamera3).get_area(), area2, atol=1e-2)
    assert np.isclose(squareCamera3.difference(squareCamera).
                      difference(squareCamera2).get_area(), area1, atol=1e-2)
    assert np.isclose(squareCamera3.difference(squareCamera2).
                      difference(squareCamera).get_area(), area1, atol=1e-2)
